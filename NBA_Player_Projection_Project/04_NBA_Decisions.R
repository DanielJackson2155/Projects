...
# sign Kyrie Irving at market value of $40 million. This fills our PG spot.
rem_cap_space = 63.8 - 40
print(rem_cap_space)
# Now we only have $23.8 million to fill other four roster spots: 2 PFs, 1 C and
# 1 SG.

# Since we are taking a chance on Kyrie. Let's be slightly more conservative
...
# with highest delta
top_5_players = plan_a_df %>%
  filter(market_value < 3.3) %>%
  arrange(desc(market_value)) %>%
  slice_head(n = 5) %>%
  select(player, pos, market_value, ws_dollar_val, delta)
print(top_5_players)
# player           pos    market_value    ws_dollar_val     delta
# Andre Drummond   C      3               11.7              8.72
# Patrick Beverley PG     3               9.62              6.62
# Cory Joseph      PG     3               7.03              4.03
# Bismack Biyombo  C      3               6.73              3.73
# George Hill      PG     3               6.56              3.56

# Assuming we can sign one more player (like we did in Plan A), let's grab Andre Drummond. He is the best
# valued player by our delta that we can get for $3 million next year. This leaves us
# with minimal suprlus cap space of $0.3 million.
